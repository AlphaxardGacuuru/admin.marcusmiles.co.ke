...
<body>
    <div class="page">
        ...
        <table>
            <tr>
                ...
                <td style="width: 28%;" class="text-right">
                    ...
                    <p class="status-line">Status: {{ strtoupper(str_replace('_', ' ', $invoice->status ?? '')) }}</p>
                </td>
            </tr>
        </table>
        ...
        <table>
            <tr>
                <td style="width: 50%;">
                    ...
                    <p class="value">{{ $invoice->project->name ?? '-' }}</p>
                </td>
                <td style="width: 50%;" class="text-right">
                    <p class="label">Invoice No: <span class="value">{{ $invoice->code ?? '-' }}</span></p>
                    <p class="label">Issue Date: <span class="value">{{ $invoice->issue_date ?? '-' }}</span></p>
                    <p class="label">Due Date: <span class="value">{{ $invoice->due_date ?? '-' }}</span></p>
                </td>
            </tr>
        </table>
        ...
        <table class="items-table">
            <thead>
                <tr>
                    ...
                    <th style="width: 20%;" class="text-right">Total (KES)</th>
                </tr>
            </thead>
            <tbody>
        ...
        <table style="margin-top: 10px;">
            <tr>
                ...
                <td style="width: 40%;" class="text-right">
                    <p class="label" style="font-size: 13px;">Grand Total: <span class="value">KES {{ number_format($invoice->total ?? 0, 2) }}</span></p>
                </td>
            </tr>
        </table>
        ...
        <p class="notes-text">{{ $invoice->notes ?? 'No notes available.' }}</p>
        ...
        <table style="margin-top: 20px;">
            <tr>
                <td width="100%">
                    <p class="footer-label">Prepared By: <span class="footer-value">{{ $invoice->createdBy->name ?? '-' }}</span></p>
                </td>
            </tr>
        </table>
