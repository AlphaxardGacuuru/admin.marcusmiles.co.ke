...
<body>
    <div class="page">
        ...
        <table>
            <tr>
                ...
                <td style="width: 28%;" class="text-right">
                    ...
                    <p class="status-line">Status: {{ strtoupper(str_replace('_', ' ', $quotation->status ?? '')) }}</p>
                </td>
            </tr>
        </table>
        ...
        <table>
            <tr>
                <td style="width: 50%;">
                    ...
                    <p class="value">{{ $quotation->project->name ?? '-' }}</p>
                </td>
                <td style="width: 50%;" class="text-right">
                    <p class="label">Quotation No: <span class="value">{{ $quotation->code ?? '-' }}</span></p>
                    <p class="label">Issue Date: <span class="value">{{ $quotation->issue_date ?? '-' }}</span></p>
                    <p class="label">Valid Until: <span class="value">{{ $quotation->expiry_date ?? '-' }}</span></p>
                </td>
            </tr>
        </table>
        ...
        {{-- Scope of Work --}}
        <p class="label" style="margin-bottom: 8px;">Scope of Work</p>
        <table class="items-table">
            <thead>
                <tr>
                    <th style="width: 50%;">Description (Phase / Task)</th>
            ...
            <tbody>
                @forelse($quotation->quotationItems as $index => $item)
                <tr>
        ...
        <table style="margin-top: 10px;">
            <tr>
                ...
                <td style="width: 40%;" class="text-right">
                    <p class="label" style="font-size: 13px;">Grand Total: <span class="value">KES {{ number_format($quotation->total ?? 0, 2) }}</span></p>
                </td>
            </tr>
        </table>
        ...
        <p class="notes-text">{{ $quotation->notes ?? 'No notes available.' }}</p>
        ...
        <table style="margin-top: 20px;">
            <tr>
                <td width="100%">
                    <p class="footer-label">Prepared By: <span class="footer-value">{{ $quotation->createdBy->name ?? '-' }}</span></p>
                </td>
            </tr>
        </table>
